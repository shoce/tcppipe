...
func main() {
	...
	for {
		...
		log.Printf("local:%s remote:%s ->", (*conn1).LocalAddr(), (*conn1).RemoteAddr())
		expOpen1.Add(1)
		...
		go func(conn1 *net.Conn) {
			...
			log.Printf("local:%s remote:%s -> local:%s remote:%s", (*conn1).LocalAddr(), (*conn1).RemoteAddr(), (*conn2).LocalAddr(), (*conn2).RemoteAddr())
			expOpen2.Add(1)
